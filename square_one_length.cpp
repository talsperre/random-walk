...
int main () {
	fastIO;
	int i, cat_r, cat_c, rat_r, rat_c, rand_move, j, k, flag, len = 0;
	for (i = 1; i*25 < 1000; i++) {
		matrix_size[i] = i * 25;
		len++;
	}
	...
	for (i = 0; i < len; i++) {
		cout << i * 25 << " " << ((double)survival_count[i])/(double)num_samples << "\n";
	}	
	return 0;
}